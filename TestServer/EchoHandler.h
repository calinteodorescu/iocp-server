...
public:
    ...
    virtual void OnNewConnection( uint64_t                     cid, 
                                  ConnectionInformation const& c
                                )
    {
        // critical section
        {
    ...
    virtual void OnReceiveData( uint64_t                    cid,
    ...
    virtual void OnSentData( uint64_t cid, 
    ...
    virtual void OnClientDisconnect( uint64_t cid,
                                     int32_t  e
                                   )
    {
        // critical section
        ...
        try
        {
            if ( true == m_sendGracefulShutdownMessage )
            {
                ...
                // correctly.
                const char* finalMessage1 = "All your base ";
                const char* finalMessage2 = "belongs to askldjd";

                // send final message 1
                GetIocpServer().Send( cid, 
                                      std::vector<uint8_t>( finalMessage1, 
                                                            finalMessage1 + strlen( finalMessage1 )
                                                          )
                                    );
                // send final message 2
                GetIocpServer().Send( cid, 
                                      std::vector<uint8_t>( finalMessage2, 
    ...
    virtual void OnDisconnect( uint64_t cid, 
                               int32_t
                             )
    {
        {
